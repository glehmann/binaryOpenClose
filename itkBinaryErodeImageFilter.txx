...
::ThreadedGenerateData(const OutputImageRegionType& outputRegionForThread,int threadId)
{
  ...
  //typename TempImageType::Pointer tmpImage = this->GetOutput();
  ...
  static const unsigned char onTag          = 50;
  static const unsigned char borderTag      = 255;                            
  static const unsigned char innerTag       = 150;
  ...
  // InputRegionForThread is the output region for thread padded by kernel radius
  // We must traverse this padded region because some border pixel in the added band
  // ( the padded band is the region added after padding )
  // may be responsible to the painting of some pixel in the non padded region.
  // This happens typically when a non centered SE is used, a kind of shift is done 
  // on the "on" pixels of image. Consequently some pixels in the added band can appear
  // in the current region for thread due to shift effect.
